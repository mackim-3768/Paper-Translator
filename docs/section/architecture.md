...
- v1.0.0 목표
  - 대상: 일반적인 1~2단 구조의 학술 논문 PDF (예: arXiv/ACM/IEEE 스타일 등)를 우선 지원.
  - 원본 PDF의 **서식과 레이아웃(다단, 줄바꿈, 폰트/크기, 컬럼/각주 위치 등)**을 최대한 그대로 유지.
  - 본문/캡션/각주 텍스트를 자연스러운 한국어로 번역.
  - 문서 전체에서 주요 용어를 **글로벌 glossary** 기반으로 일관되게 번역.
  - 수식은 가능한 경우 **LaTeX 기반으로 재구성**하여 원본과 유사한 위치에 렌더링하고, 인식이 어려운 경우 원본 이미지를 그대로 유지.

- 현실적 제약/비목표
  - 임의의 모든 PDF(포스터, 잡지형 디자인, 프레젠테이션 슬라이드 등)에 대해 픽셀 단위로 100% 동일한 레이아웃을 보장하지는 않음.
  - 복잡한 도표/그림 내부 구조까지 자동으로 재조합하는 것은 범위 밖으로 둠.

### 6.2 구현 방향

- `PDFParser`는 페이지별로 **Block 단위의 구조**를 추출합니다.
  - 타입 예: `title`, `heading(level)`, `paragraph`, `caption`, `table`, `equation` 등.
  - 각 Block의 메타데이터에는 위치(bbox), 폰트/크기, 컬럼 인덱스 등 레이아웃 정보를 포함합니다.

- 번역 시 텍스트 Block만 대상으로 하되, **수식/표/그림**은 다음과 같이 처리합니다.
  - 수식: 가능한 경우 LaTeX 형태로 추출/복원하여 텍스트 기반으로 렌더링하고, 실패 시 원본 이미지를 그대로 유지.
  - 표/그림: 캡션 텍스트는 번역하되, 실제 이미지는 원본을 사용.

- `PDFGenerator` 단계에서는
  - `PDFParser`에서 추출한 Block의 위치/컬럼/폰트 정보를 활용해, 원본 PDF와 최대한 유사한 레이아웃(다단, 문단 경계, 각주 위치 등)을 재현합니다.
  - Block 타입에 따라 스타일을 다르게 적용하되, 상대적인 크기/굵기/간격을 통해 원본의 계층 구조를 유지합니다.
